...
names(movies) = c("Kinostart", "Filmtitel", "Verleiher", "Kinderfilm", "FSK", "Genre_Prognose", "Genre1", "Genre2",
                  "Kopien", "Besucher_wochenende1", "Kopienschnitt", "Besucher_gesamt", "Tipp_min6", "Tipp_max6",
                  "Tipp_min5","Tipp_max5", "Tipp_min4", "Tipp_max4", "Tipp_min3","Tipp_max3", "Tipp_min2", "Tipp_max2",
                  "Tipp_min1", "Tipp_max1", "Tippspiel", "Awareness_unaided6", "Awareness_unaided5",
                  "Awareness_unaided4", "Awareness_unaided3", "Awareness_unaided2", "Awareness_unaided1",
                  "Awareness_aided6", "Awareness_aided5", "Awareness_aided4", "Awareness_aided3", "Awareness_aided2",
                  "Awareness_aided1", "First_choice6", "First_choice5", "First_choice4", "First_choice3", 
                  "First_choice2", "First_choice1")

movies$Filmtitel = as.character(movies$Filmtitel)
movies$FSK = as.factor(movies$FSK)
movies$Kinostart = as.POSIXct(movies$Kinostart, format = "%d.%m.%y", tz = "GMT")
movies$Kinderfilm = as.factor(ifelse(test = movies$Kinderfilm == "ja", yes = 1, no = 0))
...
movies$Prognose_start = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 7)) + 
  as.difftime(tim = 1, units = "days")
movies$Prognosedatum6 = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 6))
movies$Prognosedatum5 = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 5))
movies$Prognosedatum4 = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 4))
movies$Prognosedatum3 = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 3))
movies$Prognosedatum2 = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 2))
movies$Prognosedatum1 = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 1))

# Replace "&" with "und" for german movies and "and" for english ones
movies$Filmtitel[c(67, 95, 151, 236, 559, 608, 621, 826, 836, 846)] = 
  gsub(movies$Filmtitel[c(67, 95, 151, 236, 559, 608, 621, 826, 836, 846)], pattern = "&", replacement = "and")
...
end_forecast_week = do.call(what = c, args = lapply(X = movies$Kinostart, FUN = forecast_date, forecast = 1))
...
movies$Untertitel = as.factor(ifelse(test = main_title == complete_title, yes = 0, no = 1))

movies$Woerter = sapply(X = strsplit(x = movies$Haupttitel, split = " "), FUN = length)
movies$Monat = factor(strftime(movies$Kinostart, format = "%b"))
movies$Monat = factor(movies$Monat, levels = unique(movies$Monat))

movies$Saison = as.factor(ifelse(test = movies$Monat %in% c("Okt", "Nov", "Dez", "Jan", "Feb", "Mär"), 
                                 yes = "Winter", no = "Sommer"))
movies$Jahr_gerade = as.numeric(strftime(x = movies$Kinostart, format = "%y"))
movies$Jahr_gerade = as.factor(ifelse(test = movies$Jahr_gerade %% 2 == 0, yes = 1, no = 0))
