...
gt_anker1_median = gtrends(keyword = ankers[1], geo = "DE",  
...
for (i in 2:length(ankers)){
  assign(paste0("gt_anker", i-1, "_anker", i),
         exportGT(terms = ankers[i], startdates = startdate, enddates = enddate, anker = ankers[i-1]))
}
...
save.image("data/gt-data_ankers.RData")